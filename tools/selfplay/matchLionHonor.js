...
// Lion Honor (Kyuden Ikoma honor race) vs the Crane precon. Seats alternate to
// cancel first-player advantage. Usage:
//   node tools/selfplay/matchLionHonor.js [games] [seed] [--trace]
...
//   SUBJECT=LionHonor node tools/selfplay/deckFieldWinRate.js
// for that, per `.claude/skills/roundrobin/SKILL.md`.

const { runGame } = require('./harness.js');
const { loadLionHonorDeck, loadCraneDeck } = require('./deckLoader.js');
...
async function main() {
    ...
    for(let i = 0; i < games; i++) {
        ...
        const names = botFirst ? ['LionHonor', 'Crane'] : ['Crane', 'LionHonor'];
        const seeds = botFirst ? [botSeed, 1] : [1, botSeed];
        const decks = botFirst
            ? { deckA: loadLionHonorDeck(), deckB: loadCraneDeck() }
            : { deckA: loadCraneDeck(), deckB: loadLionHonorDeck() };
    ...
    console.log(`\nLionHonor (seed ${botSeed}) ${botWins} - ${craneWins} Crane (other ${other}), avg rounds ${(roundsTotal / games).toFixed(1)}`);
    console.log('win reasons:', JSON.stringify(reasons, null, 1));
    if(trace) {
        const sorted = Object.entries(traceHistogram).sort((a, b) => b[1] - a[1]);
        console.log('lion-honor decision reasons:');
