...
// Scorpion "Bid War" (Kyuden Bayushi honor-dial deck) vs the Crane precon.
// Seats alternate to cancel first-player advantage. Usage:
//   node tools/selfplay/matchScorpionBidWar.js [games] [bidWarSeed] [--trace]
...
//   SUBJECT=ScorpionBidWar node tools/selfplay/deckFieldWinRate.js
// for that, per `.claude/skills/roundrobin/SKILL.md`.

const { runGame } = require('./harness.js');
const { loadScorpionBidWarDeck, loadCraneDeck } = require('./deckLoader.js');
...
async function main() {
    ...
    for(let i = 0; i < games; i++) {
        ...
        const names = botFirst ? ['ScorpionBidWar', 'Crane'] : ['Crane', 'ScorpionBidWar'];
        const seeds = botFirst ? [botSeed, 1] : [1, botSeed];
        const decks = botFirst
            ? { deckA: loadScorpionBidWarDeck(), deckB: loadCraneDeck() }
            : { deckA: loadCraneDeck(), deckB: loadScorpionBidWarDeck() };
        ...
        if(result.winner === 'ScorpionBidWar') {
            botWins++;
    ...
    console.log(`\nScorpionBidWar (seed ${botSeed}) ${botWins} - ${craneWins} Crane (other ${other}), avg rounds ${(roundsTotal / games).toFixed(1)}`);
    console.log('win reasons:', JSON.stringify(reasons, null, 1));
    if(trace) {
        const sorted = Object.entries(traceHistogram).sort((a, b) => b[1] - a[1]);
        console.log('bid-war decision reasons:');
