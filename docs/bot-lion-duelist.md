...
| `+` **Frostbitten Crossing** (4) | Moderate. Conflict Action at this province: discard **every** attachment on one participant. | Playbook `inPlayAction` + `actionBeforePass`; target via `LionDuelistTactics.pickStripTarget`, which reuses the shared `AttachmentControlProfile` score tables. Our own side scores **known debuffs only** — stripping our own weapons is never the play — theirs scores every attachment. Cancels cleanly when nothing clears `stripMinimumValue`. Shipped as the **stronghold province** per the deck guide; measured against the alternatives in §5. |
...
| Frostbitten Crossing | `stripMinimumValue` |
