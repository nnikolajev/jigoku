...
|---|---|---|
| `+` **Kyuden Bayushi** | High. Bow to ready a dishonored friendly; +1/+1 at ≤6 honor. The ready is the point — a bowed participant contributes 0 skill. | `BidWarTactics.shouldUseStronghold` (the eligibility test; the ready TARGET then comes from the shared ready selector, which is why the gate matters), knobs `strongholdReadyMinSkill` / `strongholdReadyRequiresConflict` / `strongholdBandBonusOnly`, gated in `conflictAbilitySources`. **Requires a BOWED dishonored body** — an ungated click spent the once-per-round bow on a cancel (the shared ready selector cancels when nothing of ours is bowed). |
