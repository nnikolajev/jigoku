...
// Lion Duelist (Kyuden Ikoma honor-switch deck) vs the Crane precon.
// Seats alternate to cancel first-player advantage. Usage:
//   node tools/selfplay/matchLionDuelist.js [games] [seed] [--trace]
...
//   SUBJECT=LionDuelist node tools/selfplay/deckFieldWinRate.js
// for that, per `.claude/skills/roundrobin/SKILL.md`.

const { runGame } = require('./harness.js');
const { loadLionDuelistDeck, loadCraneDeck } = require('./deckLoader.js');
...
async function main() {
    ...
    for(let i = 0; i < games; i++) {
        ...
        const names = botFirst ? ['LionDuelist', 'Crane'] : ['Crane', 'LionDuelist'];
        const seeds = botFirst ? [botSeed, 1] : [1, botSeed];
        const decks = botFirst
            ? { deckA: loadLionDuelistDeck(), deckB: loadCraneDeck() }
            : { deckA: loadCraneDeck(), deckB: loadLionDuelistDeck() };
    ...
    console.log(`\nLionDuelist (seed ${botSeed}) ${botWins} - ${craneWins} Crane (other ${other}), avg rounds ${(roundsTotal / games).toFixed(1)}`);
    console.log('win reasons:', JSON.stringify(reasons, null, 1));
    if(trace) {
        const sorted = Object.entries(traceHistogram).sort((a, b) => b[1] - a[1]);
        console.log('lion-duelist decision reasons:');
