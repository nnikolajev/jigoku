...
// Crane Courtier Honor (Seven Fold Palace honor race) vs the Crane precon.
// Seats alternate to cancel first-player advantage. Usage:
//   node tools/selfplay/matchCraneHonor.js [games] [seed] [--trace]
...
//   SUBJECT=CraneHonor node tools/selfplay/deckFieldWinRate.js
// for that, per `.claude/skills/roundrobin/SKILL.md`.

const { runGame } = require('./harness.js');
const { loadCraneHonorDeck, loadCraneDeck } = require('./deckLoader.js');
...
async function main() {
    ...
    for(let i = 0; i < games; i++) {
        ...
        const names = botFirst ? ['CraneHonor', 'Crane'] : ['Crane', 'CraneHonor'];
        const seeds = botFirst ? [botSeed, 1] : [1, botSeed];
        const decks = botFirst
            ? { deckA: loadCraneHonorDeck(), deckB: loadCraneDeck() }
            : { deckA: loadCraneDeck(), deckB: loadCraneHonorDeck() };
    ...
    console.log(`\nCraneHonor (seed ${botSeed}) ${botWins} - ${craneWins} Crane (other ${other}), avg rounds ${(roundsTotal / games).toFixed(1)}`);
    console.log('win reasons:', JSON.stringify(reasons, null, 1));
    if(trace) {
        const sorted = Object.entries(traceHistogram).sort((a, b) => b[1] - a[1]);
        console.log('crane-honor decision reasons:');
