...
/** Live board facts a spec is checked against. Supplied by the caller so this
 *  module stays free of prompt plumbing and serialized-state shapes. */
export interface SequenceBoard {
    conflictType: 'military' | 'political';
    amAttacker: boolean;
    honorLead: boolean;
    selfParticipants: number;
    opponentParticipants: number;
    holdsClaimedMilitaryRing: boolean;
    hasReadyNonUniqueBody: boolean;
    hasSacrificeFodder: boolean;
    isCavalry: (card: any) => boolean;
    hasTrait: (card: any, trait: string) => boolean;
    isFaction: (card: any, faction: string) => boolean;
    printedCostOf: (card: any) => number | undefined;
    /** uuid of the character this source is attached to / is. */
    bearerUuidOf: (sourceCard: any) => string | undefined;
}

/** Can this move source move THIS character right now? */
export function moveSourceCanTake(spec: MoveSourceSpec, sourceCard: any,
    card: any, board: SequenceBoard): boolean {
    if(spec.conflictType && spec.conflictType !== board.conflictType) {
        return false;
    }
    if(spec.defendingOnly && board.amAttacker) {
        return false;
    }
    if(spec.requiresHonorLead && !board.honorLead) {
        return false;
    }
    if(spec.requiresFewerParticipants && board.selfParticipants >= board.opponentParticipants) {
        return false;
    }
    if(spec.requiresCavalry && !board.isCavalry(card)) {
        return false;
    }
    if(spec.selfOrBearerOnly && board.bearerUuidOf(sourceCard) !== String(card?.uuid || '')) {
        return false;
    }
    return true;
}

/** Can this ready source ready THIS character right now? */
export function readySourceCanTake(spec: ReadySourceSpec, card: any, board: SequenceBoard): boolean {
    if(spec.trait && !board.hasTrait(card, spec.trait)) {
        return false;
    }
    if(spec.faction && !board.isFaction(card, spec.faction)) {
        return false;
    }
    if(spec.requiresHonored && !card?.isHonored) {
        return false;
    }
    if(spec.requiresUnique && !card?.isUnique) {
        return false;
    }
    if(spec.minTargetFate !== undefined && (Number(card?.fate) || 0) <= spec.minTargetFate) {
        return false;
    }
    if(spec.maxPrintedCost !== undefined) {
        const printed = board.printedCostOf(card);
        // An unknown printed cost is not evidence the target is legal.
        if(!Number.isFinite(printed) || Number(printed) > spec.maxPrintedCost) {
            return false;
        }
    }
    if(spec.requiresClaimedMilitaryRing && !board.holdsClaimedMilitaryRing) {
        return false;
    }
    if(spec.requiresReadyNonUniqueCost && !board.hasReadyNonUniqueBody) {
        return false;
    }
    return !spec.requiresSacrificeFodder || board.hasSacrificeFodder;
}
